...
def insert_tweets(connection, tweets, batch_size=1000):
    ...
    '''
    for tweet_batch in batch(tweets, batch_size):
        if tweet_count >= 10000000:  # Check if 10 million tweets have been inserted
            break
        print(datetime.datetime.now(), 'insert_tweets count=', tweet_count)
        _insert_tweets(connection, tweet_batch)
        tweet_count += len(tweet_batch)  # Update the global counter after each batch
...
def _insert_tweets(connection,input_tweets):
    ...
    with connection.begin() as trans:
        ...
        bulk_insert(connection, 'tweets', tweets)
...
if __name__ == '__main__':
    ...
    # Loop through files and stop if 10 million tweets limit is reached
    with connection.begin() as trans:
